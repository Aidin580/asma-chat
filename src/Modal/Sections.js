...
export default function Sections({ onClose, onOpenaddNewSec, sections = [], setSections, onOpenEditSection }) {
  ...
  return (
    <div className={`modal-container-s ${visible ? 'fade-in' : 'fade-out'}`} onClick={(e) => e.stopPropagation()}>
      ...
      <div className="txt mbc">
        <p>
          <img src={edit2} className="pencil-icon" alt="add a new section" />
          مدیریت بخش ها&nbsp;
        </p>
      ...
      <div className="sections-list">
            ...
            {sections.map(section => (
              <div key={section.id} className={`${section.className} sec-list-item`}>
                <div className="section-logo-container">
                  <img className="section-logo-img" src={section.logo} alt="section logo" />
                </div>
        ...
        <div className="add-one-btn-container">
          <RippleEffect className="make-sec" onClick={onOpenaddNewSec}>
            ساخت بخش جدید<img src={add} alt="add new section button" />
          </RippleEffect>
        </div>
      </div>
    </div>
